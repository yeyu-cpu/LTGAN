...
from face_parsing import test
from PIL import Image
from GeomConsistentFR import resize
...
class RelightNet(nn.Module):
    ...
    def forward(self, img, epoch, intrinsic_matrix, mask, target_lighting, target_ambient_values):
        ...
        for i in range(self.batch_size):
            ...
            light_x = light_xy[0, 0, 0].cpu().detach().numpy().item()# np.asscalar(light_xy[0, 0, 0].cpu().detach().numpy())
            light_y = light_xy[1, 0, 0].cpu().detach().numpy().item()#np.asscalar(light_xy[1, 0, 0].cpu().detach().numpy())
...
def main_lighting_trasfer(reference,source,reference_mask):
    model = RelightNet()
    ...
    reference_path_all = reference.split('/')
    reference_path = reference_path_all[0]
    for i in range(1,len(reference_path_all)-1):
        reference_path += '/'
        reference_path += reference_path_all[i]
    test.evaluate(dspth=reference_path,respth='GeomConsistentFR/FFHQ_skin_masks')
    input_mask = resize.resize_image(reference_mask)
  
    input_image = f'{reference}'
    reference_image = f'{source}'
    ...
    with torch.no_grad():
        curr_input_image = torch.reshape(torch.from_numpy(cv2.resize(imageio.imread(input_image)/255.0, (256,256))), (1, 256, 256, 3))
        curr_reference_image = torch.reshape(torch.from_numpy(cv2.resize(imageio.imread(reference_image)/255.0, (256,256))), (1, 256, 256, 3))
        curr_training_lighting = torch.from_numpy(np.zeros((model.batch_size, 4)))
        curr_img_name = input_image
        curr_mask_fill_nose = torch.from_numpy(np.reshape(input_mask, (256, 256, 1)))/255.0
        ...
        name_parts = input_image.split('/')[-1].split('.')
        ...
        cv2.imwrite('GeomConsistentFR/lighting_transfer_result/'+name_parts[0]+'_rendered_image.png', input_image)

            
if __name__ == '__main__':
    main_lighting_trasfer()
