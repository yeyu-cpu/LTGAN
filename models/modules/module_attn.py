...
class MultiheadAttention_weight(nn.Module):
    def __init__(self, feature_dim, proj_dim, num_heads=1, dropout=0.0, bias=True):
        ...
        self.k_proj = nn.Linear(feature_dim * 2 - 136, proj_dim, bias=bias)
    ...
    def forward(self, fea_c, fea_s, mask_c, mask_s):
        ...
        bsz, dim_s, h, w = fea_s.shape

        fea_c = fea_c.view(bsz, dim, h*w).transpose(1, 2) # (b, HW, d)
        fea_s = fea_s.view(bsz, dim_s, h*w).transpose(1, 2)
        with torch.no_grad():
            if mask_c.shape[2] != h:
...
class DeconvNet(nn.Module):  #for HiloAttention
    def __init__(self):
        super(DeconvNet, self).__init__()
        self.deconv = nn.ConvTranspose2d(768, 3, kernel_size=(32, 32), stride=(32, 32), bias=False)
